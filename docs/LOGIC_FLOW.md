...
[1] Is file readable?
|
+--NO--> ERROR: File corrupted --> VERDICT: CORRUPTED
|
+--YES
|
v
[2] Extract metadata (sample rate, channels, duration)
|
v
[3] Perform spectral analysis (FFT)
|
+--ERROR--> VERDICT: CORRUPTED
|
+--SUCCESS
|
v
[4] Calculate cutoff frequency & energy ratio
|
v
[5] Calculate bitrate metrics
|
v
[6] Check quality issues (duration, DC offset, silence, clipping)
|
v
[7] Check upsampling
|
v
[8] Apply 11 detection rules --> SCORE (0-150 points)
|
v
[9] Determine verdict based on score
|
v
END: Report result
...
THEN SKIP RULE (probably FFT rounding, not MP3)
RETURN 0 points
...
THEN SCORE += 50 points
AND SET estimated_mp3_bitrate
...
THEN SCORE += 50 points (inflated file)
...
IF diff > 1.0 second
    ...
    THEN SCORE += 20 points (encoding error)
...
THEN SCORE += 25 points (fake hi-res)
...
FOR each expected_cutoff:
    IF hard_cutoff_detected_at(expected_cutoff)
        ...
        THEN is_upsampled = TRUE
        AND suspected_original_rate = expected_cutoff * 2
...
CORRUPTED                    |
  ...
  SCORE>=80    SCORE>=50    SCORE<30 |
      |             |             |   |
      v             v             v   v
FAKE_CERTAIN  FAKE_PROBABLE  AUTHENTIC
    [XX]          [!!]          [OK]
     |             |             |
