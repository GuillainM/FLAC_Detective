...
def detect_vinyl_noise(
    ...
    # Filter band above cutoff (up to Nyquist)
    ...
    rms_energy = np.sqrt(np.mean(noise_band ** 2))
    energy_db = 20 * np.log10(rms_energy + 1e-10)
    details['energy_db'] = float(energy_db)
    ...
    segment_length = min(len(noise_band), int(sample_rate * 1.0))  # 1 second
    segment = noise_band[:segment_length]
    
    # Normalize
    segment = segment - np.mean(segment)
    segment = segment / (np.std(segment) + 1e-10)
    
    # Autocorrelation at lag 50 samples (~1ms at 44.1kHz)
    lag = 50
    if len(segment) > lag * 2:
        autocorr = np.corrcoef(segment[:-lag], segment[lag:])[0, 1]
        details['autocorr'] = float(np.abs(autocorr))
        
        logger.debug(f"VINYL: Autocorrelation @ {lag} samples = {autocorr:.3f}")
    else:
        details['autocorr'] = 0.0
    
    # Vinyl noise is random (low autocorrelation < 0.3)
    # Patterned noise (dither) has higher autocorrelation
    
    # 3. Measure temporal constancy (variance across segments)
    # Divide into 5 segments of 1 second each
    segment_duration = 1.0  # seconds
    segment_samples = int(segment_duration * sample_rate)
    num_segments = min(5, len(noise_band) // segment_samples)
    
    if num_segments < 2:
        logger.debug("VINYL: Audio too short for temporal analysis")
        details['temporal_variance'] = 0.0
    else:
        segment_energies = []
        for i in range(num_segments):
            start_idx = i * segment_samples
            end_idx = start_idx + segment_samples
            seg = noise_band[start_idx:end_idx]
            seg_rms = np.sqrt(np.mean(seg ** 2))
            seg_db = 20 * np.log10(seg_rms + 1e-10)
            segment_energies.append(seg_db)
        
        temporal_variance = np.std(segment_energies)
        details['temporal_variance'] = float(temporal_variance)
